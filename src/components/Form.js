...
export function Input({
  ...
  register = undefined,
  defaultValue = "",
  value = "", // Ensure value starts empty by default
  onChange,
}) {
  return (
    <div className="text-sm w-full">
      <label className={`${color ? 'text-black text-sm' : 'text-white font-semibold'}`}>
        {label}
      </label>
      <input
        ...
        {...(register ? register(name) : {})} // Optional usage of register
