...
const SearchUser = ({ onClose }) => {
  ...
  return (
    <div className="fixed top-0 bottom-0 left-0 right-0 bg-slate-700 bg-opacity-40 p-6 sm:p-10 z-10">
      <div className="w-full max-w-lg mx-auto mt-20 pt-6">
        ...
        <div className="bg-white mt-2 w-full p-4 rounded">
          ...
          )}
          {
            searchUser.length !== 0 && !loading && (
              searchUser
                .filter(user => user.role === "ADMIN") // Only show users with ADMIN role
                .map(user => (
      ...
      <div className="absolute mt-20 pt-4 top-0 -right-0 p-2 text-2xl lg:text-4xl hover:text-white" onClick={onClose}>
        <button>
          <IoClose />
        </button>
      </div>
